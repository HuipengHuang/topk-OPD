...
# def labeling_responses(responses: list[str], golden_answer: str):
#     predict_answers = list(map(lambda x: parse(x, parsing_timeout=10.0), responses))
#     golden_answers = list(map(lambda x: parse(x, parsing_timeout=10.0), ["$" + golden_answer + "$"] * len(responses)))
#     labels = list(map(lambda g, p: verify(g, p, timeout_seconds=10.0), golden_answers, predict_answers))
#     return labels
...
def reward_fn_math_verify_no_think(solution_str: str, ground_truth: Union[str, List[str]], enable_llm=False):
    # print(f"coming to math verify no think\n")
    reward_config = RewardConfig()
    ...
    reward_response = reward_fn(
        RewardInput(problem=solution_str, problem_type=RewardType.MATH, model_response=solution_str,
                    ground_truth={"answer": ground_truth}))
    # print(reward_response.is_correct)
    return reward_response.is_correct
